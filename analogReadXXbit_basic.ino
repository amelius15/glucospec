...
const uint8_t pin1 = A1; //analogRead pin
...
void loop() 
{
  ...
  float analog_reading = adc.analogReadXXbit(pin1,10,5);
//  Serial.println(analog_reading);
  ...
  else
  {
    ...
    if(analog_reading<975.0)
    {  
      digitalWrite(ledPin, HIGH);
      if(!yes)
        {
          num_samples = 10;
          do_a_bunch_of_samples(num_samples, pin);
          pos+=1;
          yes = true;
...
void do_a_bunch_of_samples(unsigned long num_samples, uint8_t pin)
{
  //local variables
